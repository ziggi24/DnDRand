This is a python code to create randomized DnD character sheets using the DnD 1.0 style. 

This code will continue to create semi-formatted character sheets until the user simply clicks enter on the name prompt

this is version 1.0 of the code


TO BE ADDED
~~~~~~~~~~~

- comments to the python script
- random weapon based on class 
- random apparel 
- larger pool of names and titles (DONE)
- list of last names added and random last name assigned (DONE)
- find players top two stats and better sort them into classes
- small backstory for each character (still being considered) 
- anything else?
